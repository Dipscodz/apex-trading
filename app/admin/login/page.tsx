...
export default function AdminLoginPage() {
  ...
  if (currentUser?.role === 'admin') {
    return (
      <div className="max-w-md mx-auto my-16 glass-panel p-8 rounded-3xl border border-emerald-500/40 text-center space-y-4">
        ...
        <p className="text-xs text-slate-300">You hold active Chief Administrator credentials.</p>
        <button
          onClick={() => router.push('/admin')}
          className="w-full py-2.5 rounded-xl font-bold text-xs bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg"
        >
          Proceed to Admin Executive Console →
        </button>
      </div>
    );
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (res.success) {
      setSuccess('Admin Security Clearance Authorized');
  ...
  return (
    <div className="max-w-md mx-auto my-12 glass-panel rounded-3xl border border-slate-700/80 p-8 space-y-6 shadow-2xl">
      <div className="text-center space-y-2">
        ...
        <h1 className="text-2xl font-extrabold text-white tracking-tight">Executive Admin Gate</h1>
